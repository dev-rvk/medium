            ...
            <Link to={`/blog/${postData.id}`}>
                <div className="flex flex-col px-3 py-3 w-full mt-4 mb-1 border-2 rounded-lg shadow-lg">
                    <div className="flex flex-row items-center mt-2">
                        <div className="ml-4"> <Avatar  size="large" text={postData.authorName[0]} /> </div>
                    ...
                    <div className="ml-4 mt-4 font-black text-2xl"> {postData.title} </div>
                    <div className="ml-4 mt-2 font-extralight text-xl text-gray-500"> {postData.content.substring(0, 150) + (postData.content.length < 100 ? "" : "...")} </div>
                    <div className="ml-4 mt-6 font-extralight text-gray-400"> { minsRead + (minsRead > 1 ? " minutes read" : " minute read") } </div>
                </div>
            </Link>
        </div>
